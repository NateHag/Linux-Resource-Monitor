...
class NateSysTray:
    ...
    def draw_bar(self, label, pct, width=15):
        """Creates an aligned ASCII bar by using fixed-width padding."""
        pct = int(float(pct))
        filled = int((pct / 100) * width)
        bar = "█" * filled + "░" * (width - filled)
        
        # Using a wider 12-character padding for the label ensures the '|' 
        # symbols always start at exactly the same horizontal position.
    ...
    def update_data(self):
        try:
            ...
            # --- BRAIN ---
            self.item_cpu.set_label(self.draw_bar("CPU Load", cpu))
            self.item_ram.set_label(self.draw_bar("RAM Usage", ram.percent))
            
            # --- MUSCLE ---
            gpu_index = 3
            for g in gpus:
                lbl = f"GPU: {g['model']}"
                # Using a consistent "GPU" prefix or name
            ...
            for part in psutil.disk_partitions(all=False):
                if part.mountpoint == '/' or part.mountpoint.startswith(('/mnt', '/media')):
                    ...
                    lbl = f"Disk: {name}"
                    # Truncate label to 12 to ensure fit, but draw_bar handles padding
                    bar_text = self.draw_bar(name[:12], usage.percent)
    ...
    def sync_dashboard(self, cpu, ram, gpus, io):
        ...
        for line in lines:
            if 'id="cpu-val"' in line: line = f'                        <span id="cpu-val">{cpu}%</span>\n'
                ...
                nv = [g for g in gpus if g['name'] == "NVIDIA"][0]
